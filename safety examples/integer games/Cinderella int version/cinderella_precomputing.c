...
int main() {
    ...
    int stepmother[ITERATIONS][NUM_BUCKETS] = {0};

    // computing stepmother moves before hand
    for(int i = 0; i < ITERATIONS; i++) {
        int water_remaining = STEPMOTHER_WATER; 
        for (int j = 0; j < NUM_BUCKETS - 1; j++) {
            int added_water = __VERIFIER_nondet_int(); // amount of water to be added in jth bucket of ith round by stepmother
